...
class TurnOnLight(job: Job, private val networkRepository: NetworkRepository.NetworkManager) :
    UseCase<Success.GoodRequest, Int>(job) {
    override suspend fun run(params: Int): Either<Failure, Success.GoodRequest> = networkRepository.turnOnLight(params)
}